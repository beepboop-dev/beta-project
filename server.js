...
app.get('/api/public/menu/:slug', (req, res) => {
  ...
  if (!db.menu_translations) db.menu_translations = {};
  const translations = db.menu_translations[menu.id] || {};

  // Include translations
  if (!db.menu_translations) db.menu_translations = {};
  const menuTranslations = db.menu_translations[menu.id] || {};
...
app.get('/api/menus/:menuId/specials', requireAuth, (req, res) => {
  ...
  const specials = db.specials[req.params.menuId] || { days: {}, customSpecials: [], happyHour: { enabled: false, start: '16:00', end: '18:00', label: 'Happy Hour', days: [1,2,3,4,5] } };
  // Migrate: ensure customSpecials exists
  if (!specials.customSpecials) specials.customSpecials = [];
  res.json({ specials });
});
...
app.get('/api/public/menu/:slug/specials', (req, res) => {
  ...
  if (!menu) return res.status(404).json({ error: 'Menu not found' });
  if (!db.specials) db.specials = {};
  const specials = db.specials[menu.id] || { days: {}, customSpecials: [], happyHour: { enabled: false, start: '16:00', end: '18:00', label: 'Happy Hour', days: [1,2,3,4,5] } };
  
  const today = new Date().getDay(); // 0=Sun
  
  // Item-based specials for today
  ...
  // Custom standalone specials for today
  const customSpecials = (specials.customSpecials || []).filter(cs => {
    return cs.days && cs.days.includes(today) && cs.isActive !== false;
  }).map(cs => ({
    id: cs.id,
    name: cs.name,
    description: cs.description || '',
    price: cs.price,
    specialPrice: cs.price, // custom specials ARE the special price
    originalPrice: cs.originalPrice || null,
    specialLabel: cs.label || 'Special',
    endTime: cs.endTime || null,
    image_url: cs.image_url || '',
    type: 'custom'
  }));
  
  res.json({ specials, todayItems: [...customSpecials, ...itemDetails], happyHour: specials.happyHour });
});
...
// ============ TRANSLATIONS ============
const { SUPPORTED_LANGUAGES, translateText: transText } = require('./translations');
...
app.get('/api/menus/:id/translations', requireAuth, (req, res) => {
  const db = load();
  const menu = db.menus[req.params.id];
  if (!menu || menu.user_id !== req.session.userId) return res.status(404).json({ error: 'Menu not found' });
  if (!db.menu_translations) db.menu_translations = {};
  const translations = db.menu_translations[req.params.id] || {};
  const languages = menu.languages || [];
  res.json({ translations, languages, supported: SUPPORTED_LANGUAGES });
});

// Set menu languages
app.put('/api/menus/:id/languages', requireAuth, (req, res) => {
  const db = load();
  const menu = db.menus[req.params.id];
  if (!menu || menu.user_id !== req.session.userId) return res.status(404).json({ error: 'Menu not found' });
  const { languages } = req.body; // array of lang codes
  menu.languages = (languages || []).filter(l => SUPPORTED_LANGUAGES[l]);
  save(db);
  res.json({ success: true, languages: menu.languages });
});

// Auto-translate a menu to a target language
app.post('/api/menus/:id/translate', requireAuth, (req, res) => {
  const db = load();
  const menu = db.menus[req.params.id];
  if (!menu || menu.user_id !== req.session.userId) return res.status(404).json({ error: 'Menu not found' });
  const { language } = req.body;
  if (!language || !SUPPORTED_LANGUAGES[language]) return res.status(400).json({ error: 'Unsupported language. Supported: ' + Object.keys(SUPPORTED_LANGUAGES).join(', ') });

  if (!db.menu_translations) db.menu_translations = {};
  if (!db.menu_translations[req.params.id]) db.menu_translations[req.params.id] = {};

  const translations = {};
  // Translate categories
  const cats = Object.values(db.categories).filter(c => c.menu_id === req.params.id);
  for (const cat of cats) {
    translations[cat.id] = {
      name: transText(cat.name, language),
      description: transText(cat.description || '', language),
    };
  }
  // Translate items
  const catIds = new Set(cats.map(c => c.id));
  const items = Object.values(db.items).filter(i => catIds.has(i.category_id));
  for (const item of items) {
    translations[item.id] = {
      name: transText(item.name, language),
      description: transText(item.description || '', language),
    };
  }

  db.menu_translations[req.params.id][language] = translations;

  // Auto-add language to menu's languages list
  if (!menu.languages) menu.languages = [];
  if (!menu.languages.includes(language)) menu.languages.push(language);

  save(db);
  res.json({ success: true, language, translations, translatedCount: Object.keys(translations).length });
});

// Update a single translation
app.put('/api/menus/:menuId/translations/:lang/:itemId', requireAuth, (req, res) => {
  const db = load();
  const menu = db.menus[req.params.menuId];
  if (!menu || menu.user_id !== req.session.userId) return res.status(404).json({ error: 'Menu not found' });
  if (!db.menu_translations) db.menu_translations = {};
  if (!db.menu_translations[req.params.menuId]) db.menu_translations[req.params.menuId] = {};
  if (!db.menu_translations[req.params.menuId][req.params.lang]) db.menu_translations[req.params.menuId][req.params.lang] = {};
  const { name, description } = req.body;
  db.menu_translations[req.params.menuId][req.params.lang][req.params.itemId] = { name, description };
  save(db);
  res.json({ success: true });
});

// Remove a language from menu
app.delete('/api/menus/:id/translations/:lang', requireAuth, (req, res) => {
  const db = load();
  const menu = db.menus[req.params.id];
  if (!menu || menu.user_id !== req.session.userId) return res.status(404).json({ error: 'Menu not found' });
  if (db.menu_translations && db.menu_translations[req.params.id]) {
    delete db.menu_translations[req.params.id][req.params.lang];
  }
  if (menu.languages) {
    menu.languages = menu.languages.filter(l => l !== req.params.lang);
  }
  save(db);
  res.json({ success: true });
});
